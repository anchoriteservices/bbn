...
docker run --privileged --cap-add=ALL --device-cgroup-rule="b 7:* rmw" --security-opt="seccomp=unconfined" -d -ti -e "container=docker" -v $WORK_DIR:rw -v /dev:/dev $DOCKER_IMAGE /bin/bash
