#!/bin/bash -xe
{
  ...
  chroot $mkRoot /bin/bash -xe <<EOF
    set -x; set -e; cd /install-scripts; export LMBUILD="raspios"; export BBN_KIND="$BBN_KIND"; ls; chmod +x *.sh; ./install.sh 0 2 4 6 8 a b; exit
EOF
  ...
  if [ "$BBN_KIND" == "LITE" ] ; then
    BBN_IMG=lysmarine-bbn-lite-bookworm_${thisArch}-${cpuArch}.img
  else
    BBN_IMG=lysmarine-bbn-full-bookworm_${thisArch}-${cpuArch}.img
  fi
