...
if [ -f /boot/cmdline.txt ]; then
  sed -i '$s/$/\ console=tty1\ loglevel=1\ splash\ logo.nologo\ cfg80211.ieee80211_regdom=US\ vt.global_cursor_default=1\ plymouth.ignore-serial-consoles\ console=tty3\ cgroup_memory=1\ cgroup_enable=memory\ systemd.run=/boot/firmware/firstrun.sh\ systemd.run_success_action=reboot\ systemd.unit=kernel-command-line.target/' "$(realpath /boot/cmdline.txt)"
